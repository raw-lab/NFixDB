...
def analyze_fasta(input_topfasta, output_filteredfasta):

        df = pd.DataFrame(pd.read_table(input_topfasta))
        print("fasta counts: ")
        ...
        merged_df.to_csv(output_filteredfasta, sep="\t", index=True, index_label="GenomeID")

        return output_filteredfasta
